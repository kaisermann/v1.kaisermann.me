...
function Kaisermann() {
  ...
  var sections = {
      ...
      main: function () {
        if (!isSmallScreen()) {
          ...
          $piao.on("click", function () {
            if ($piao.hasClass("spinning_top--spinning")) {
              ...
              $iframe.css(
                "transform",
                "scale(" +
                  $monitor_window.outerWidth() / $iframe.outerWidth() +
                  ")"
              );
          ...
          $(".monitor__cam").on("click", function (e) {
            ...
            if ($batpug.hasClass("easteregg__batpug--active")) return;
    ...
    general: {
      main: function () {
        ...
        function setCurrentLanguage(lang) {
          var _b = $("body");
          _b.removeClass("lang--" + _b.attr("data-lang"))
            .attr("data-lang", lang)
            .addClass("lang--" + lang);
          cur_lang = lang;
        ...
        $flags.on("click", function () {
          ...
          if (!langs.indexOf(target_lang) < 0) return;
        ...
        if (!isMobile.any) {
          $speakers.on("click", function () {
            if (is_song_playing && $(this).hasClass("speaker--playing")) {
              ...
              if ($laptop.hasClass("animating")) playSound("typing", true);
            } else {
              var songid,
                max = songs_id.length;
              ...
              if (++current_song_index == max) current_song_index = 0;

              if ($laptop.hasClass("animating")) stopSound("typing");
            }
        ...
        (function updateClock() {
          ...
          $clock
            .find(".clock__second")
            .css("transform", "rotate(" + (angle * second - 90) + "deg)");
          $clock
            .find(".clock__minute")
            .css("transform", "rotate(" + (angle * minute - 90) + "deg)");
          $clock
            .find(".clock__hour")
            .css("transform", "rotate(" + (hourAngle - 90) + "deg)");
      ...
      queue: {
        50: function () {
          $workspace.addClass("workspace--slided");
        },
        400: isMobile.any
          ? null
          : function () {
              this.parameters.lamp
                ...
                .addClass("lamp--blink");
              if (!laptop_zoomed) playSound("click");
            },
        500: function () {
          var self = this;
          ...
          if (!isMobile.any && !laptop_zoomed) playSound("typing", true);
          ...
          animation_timers["typing"] = setTimeout(
            (function () {
              ...
              return function timer_typing_callback() {
                animation_timers["typing"] &&
                  clearTimeout(animation_timers["typing"]);

                $(self.parameters.code_tags[code_counter++]).attr(
                  "style",
                  function (i, style) {
                ...
                if (code_counter <= code_length)
                  animation_timers["typing"] = setTimeout(
                    timer_typing_callback,
                    code_interval
                  );
                else {
                  (function typeEnded() {
                    $laptop.addClass("animated").removeClass("animating");
                    if (!isMobile.any) stopSound("typing");
                  })();
                }
              };
            })(),
    ...
    about: {
      main: function () {
        ...
        var skill_animated = false,
          language_animated = false,
          list_animated = false;
        ...
        self.methods.scrollHandler = function (e) {
          ...
          if ($rowfirst.isOnScreen() && !skill_animated) {
            ...
            setTimeloop(
              ...
              150,
              function () {
                return ++skills_cur_item == $skill_items.length;
              }
            );
            skill_animated = true;
          }
        ...
        animateItems(this, 3, function (i) {
          return 300 * (i - 1);
        });
      },
      methods: {},
    ...
    contact: {
      main: function () {
        ...
        $(form_selector + " input," + form_selector + " textarea").on(
          "input",
          function () {
            $(this)
              .parent()
        ...
        $(form_selector + " .form__submit").on("click", function (a) {
          a.preventDefault();
          for (
            var b = $(form_selector).find(".form__submit"),
              c = $(form_selector),
              d = c.serializeArray(),
              e = [],
              f =
            ...
            var h = d[g];
            h.value.length < 1
              ? e.push({ name: h.name, msg: getLang("empty_field") })
              : "email" != h.name ||
                f.test(h.value) ||
                e.push({ name: h.name, msg: getLang("invalid_email") });
          }
          return e.length
            ? (e.forEach(function (a) {
              ...
              void scrollToElem('[name="' + e[0].name + '"]', 300, -50))
            : (b.addClass("form--sending"),
              void $.ajax({
                type: "POST",
                url: "functions/sendMail.php",
                data: c.serialize() + "&lang=" + cur_lang,
                success: function (a) {
                  (a = $.parseJSON(a)), b.removeClass("form--sending");
                  var c = document.createElement("div");
                  (c.className =
                    "form__feedback feedback--" +
                    (1 == a.state ? "ok" : "error")),
                    (c.innerHTML = a.msg),
                    $(c).hide().insertAfter(form_selector).fadeIn(),
                    scrollToElem(".form__feedback", 300, 50);
                },
              }));
        });
    ...
    projects: {
      main: function (parameters) {
        ...
        function toggleCompleteInfo(a, b) {
          b
            ? isSmallScreen()
              ? a.show()
              : a.slideDown()
          ...
          if (!_complete.data("slided")) {
            ...
            gal.on("cellSelect", function () {
              ...
              gal
                ...
                .css("max-height", cur_slide.outerHeight());
            });
          }
        ...
        if (this.parameters.hash !== undefined) {
          ...
          this.parameters["focused_project"] = $(
            '.project__link[href$="#projects/' + project + '"]'
          );
        }
      },
      onQueueStep: function (step_selector) {
        if (
          ...
          this.parameters["focused_project"]
            .parent()
            .hasClass(step_selector.substring(1))
        )
  ...
  var events = {
    runSection: function (id, queue_id) {
      ...
      if (!section.parameters) section.parameters = {};

      if (sections[id].hasOwnProperty("main")) section.main.call(section);
        ...
        (function sectionQueueHelper(id, queue, limit, current, interval) {
          ...
          if (page.queue.hasOwnProperty(current)) {
            ...
            if (typeof current_queue_item == "function")
              ...
              if (
                current_queue_item &&
                typeof current_queue_item.method == "function"
              )
                step_ret = current_queue_item.method.call(
                  page,
                  current_queue_item.params
                );
            }
          ...
          if (limit) {
            current += interval;
            animation_timers[id] = setTimeout(function () {
              sectionQueueHelper(id, queue, limit, current, interval);
            }, interval);
          } else {
    ...
    openSection: function (hash_array) {
      ...
      if (!sections.hasOwnProperty(section_name)) return;
  ...
  function init() {
    ...
    if (!isMobile.any) {
      for (var i = 0; i < sounds_id.length; i++)
        sounds[sounds_id[i]] = new Audio(
          assets_url + "sound/" + sounds_id[i] + ".mp3"
        );
    }
  ...
  function animateItems(obj, max, calc_or_offset, inc) {
    ...
    if (typeof calc_or_offset == "function") {
      for (var i = 1; i <= max; i++)
        obj.queue[calc_or_offset(i)] = {
          params: { id: i },
          method: function (params) {
            showStructure(".animation--" + params.id);
            return ".animation--" + params.id;
          },
        };
    } else {
  ...
  function showStructure(selector) {
    $(selector)
      .removeClass("structure--invisible")
      .addClass("structure--visible");
  }

  function isSmallScreen() {
    return isMobile.any || win.width < 992;
  }

  function playSound(id, shouldLoop) {
    shouldLoop = !!shouldLoop;

    if (shouldLoop) sounds[id].addEventListener("ended", sound_looper_callback);
    sounds[id].play();
  }
  ...
  function sound_looper_callback() {
    this.currentTime = 0;
    this.play();
  }
  ...
  function applyTransforms(el) {
    var laptopArea = el[0].querySelector(".monitor__screen"),
      laptopAreaSize = {
        width: laptopArea.offsetWidth,
        height: laptopArea.offsetHeight,
      },
      scaleVal =
        laptopAreaSize.width / laptopAreaSize.height < win.width / win.height
          ? win.width / laptopAreaSize.width
          : win.height / laptopAreaSize.height;
